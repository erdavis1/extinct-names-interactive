...
<script>
   ...
   * @property {boolean} [xBaseline=false] - Show a solid line at the bottom.
   * @property {boolean} [yBaseline=false] - Show a solid line at the left.
...
<g class="axis x-axis" class:snapLabels>
  {#each tickVals as tick, i (tick)}
    ...
    {#if xBaseline === true && i===0}
     <g transform="translate({$xScale(tick)},{Math.max(...$yRange)})">
        <line class="baseline"  x1={halfBand} x2={halfBand} y1={-$height} y2="0" />
     </g>
    {/if}
